...
def spit_plot_diff_random_seeds_naf2_ppo(param, ylabel):
	fig, ax = plt.subplots()
	save_path = f'NAF2-PPO_{param}.pdf'

	for algo, c, maxc, k in zip(['NAF2', 'PPO'], ('b', 'tab:orange'), ('k', 'r'), (0.9, 1.1)):
		par_dir = algo
		...
		x = desired_steps
		ax.plot(x, param_mea, label=stat_type_label, color=c)
		ax.fill_between(x, param_min, param_max, alpha=0.5, label=f'{algo} Min-Max', color=c)
		...
		if param == 'ep_ret':
			...
			maxmedret = max(param_mea)
			step_maxmedret = x[np.argmax(param_mea)]
			ax.axhline(maxmedret, color=maxc, label=f'Max({stat_type_label})', lw=0.6)
			ax.axvline(step_maxmedret, color=maxc, lw=0.3)

			trans = transforms.blended_transform_factory(
				ax.get_yticklabels()[0].get_transform(), ax.transData)
			ax.text(0, k*maxmedret, f"{maxmedret:.1f}", color=maxc, transform=trans,
			        ha="right", va="center")
			trans = transforms.blended_transform_factory(
				ax.transData, ax.get_xticklabels()[0].get_transform())
			ax.text(step_maxmedret, 0.05, f"{int(np.round(step_maxmedret))}", color=maxc, transform=trans,
			        ha="center", va="center")

	ax.grid(axis='both', which='both', alpha=0.3)
	ax.set_ylabel(ylabel)
	ax.set_xlabel('Training Steps')
	ax.set_title('NAF2 & PPO training')
	ax.legend(loc='best')
...
def spit_plot_diff_best_hparams(algo, param, ylabel, index_to_agent_hint, par_dir=None):
	...
	desired_steps = np.arange(0, 80001, 100)
...
def plot_all_agents():
	...
	par_dir = 'D:\Code\\rl-qfb\\results\\training_default_params'
	...
	for param, ylabel in zip(params, ylabels):
		spit_plot_diff_random_seeds_naf2_ppo(param, ylabel)
...
def plot_all_hparam_agents():
	for param, ylabel in zip(('ep_len', 'ep_ret', 'succ'),
		...
		spit_plot_diff_best_hparams(algo='SAC',
		                            param=param,
		                            ylabel=ylabel,
		                            index_to_agent_hint={0: '2244', 1: '2055', 2: '0928', 3: '0049', 4: '0011'},
